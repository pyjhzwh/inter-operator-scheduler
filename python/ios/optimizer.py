    ...
    for bindex, block in enumerate(graph.blocks):
        ...
        for part_index, npart in enumerate(node_parts):
            ...
            dop(ustate, block, chains, on_debug, debug_dp_info, idn, nid, dp, ep, opt_type, max_group_size,
                ...
                cost_model, batch_size, warmup, number, repeat, bar_state, transform_blacklist)
...
def get_transform_latency_single_conv(
    ...
    if len(stage_seqs) == 1 and len(stage_seqs[0]) == 1:
        if isinstance(idn[stage_seqs[0][0]], Conv):
            ...
            layouts = ["NCHW", "NHWC"]
            exe_time_layout_map = {}
            # print(f"IN_C:{terms[0][0].node.output_shape[0]}, IN_H: {terms[0][0].node.output_shape[1]}, IN_W: {terms[0][0].node.output_shape[2]},",
            #     f"OUT_C: {out_channels}, KERNEL_H: {kernel[0]}, KERNEL_W: {kernel[1]},",
            #     f"STRIDE_H: {stride[0]}, STRIDE_W: {stride[1]}, PAD_H: {padding[0]}, PAD_W: {padding[1]}")
            ...
            # print(f"Conv{nd.name}, exe_time_layout_map", exe_time_layout_map)
...
def latency(stage: Tuple[List[List[int]], str], block, merge_latency, parallel_latency, transform_latency, merge_transform_latency, cost_model, idn, nid,
            batch_size, warmup, number, repeat, try_transform: bool=False, transform_blacklist: Optional[List[List[int]]]=None) -> float:
    """
...
def dop(s: int,
        ...
        warmup, number, repeat, bar_state, transform_blacklist) -> float:
    """
...
def construct(stage_list: List[Tuple[List[List[int]], str]], block, constructed_blocks, graph_enter, idn, nid,
    ...
    for stage_seqs, qtype in stage_list:
        if "merge" in qtype and len(stage_seqs) > 1:  # only merge convolutions
            ...
            layouts = qtype.split("_", maxsplit=1)[1]
            layouts = layouts.split("|")
            conv_cnt = 0
            seq_in_stage = []
            for stage_seq in stage_seqs:
